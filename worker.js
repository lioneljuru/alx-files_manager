...
userQueue.process(async (job, done) => {
	...
	const user = await (await dbClient.usersCollection())
	  .fincOne({ _id: new mongoDBCore.BSON.ObjectId(userId) });
	if (!user) {
		throw new Error('User not found');
	}
	console.log(`Welcome ${user.email}!`);
	try {
		const mailSubject = 'Welcome to ALX-Files_Manager by lioneljuru';
		const mailContent = [
			'<div>',
			'<h3>Hello {{user.name}},</h3>'
			'Wecone to <a href="https://github.com/lioneljuru/alx-files_manager">',
			'ALX-Files_Manager</a>, ',
			'a simple file management API built with Node.js by ',
			'<a href="https://github.com/lioneljuru">JURU Jean Lionel</a>
			'We hope it meest your needs.',
			'</div>',
		].join('');
